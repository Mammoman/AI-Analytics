...
@Component({
  ...
  template: `
    <div class="rounded-xl bg-white ring-1 ring-slate-200 dark:bg-slate-900 dark:ring-white/5 p-4">
      ...
      <ol class="flex flex-col divide-y divide-slate-200 dark:divide-white/5">
        <li *ngFor="let model of models; let i = index" class="flex items-center gap-3 py-2 text-sm">
          <span class="text-slate-500 dark:text-slate-500 w-5 text-right">{{ i + 1 }}</span>
          <span class="flex-1 text-slate-700 dark:text-slate-200 truncate">{{ model.name }}</span>
          <span class="text-slate-900 dark:text-white font-medium">{{ model.score }}</span>
          <span
